...
- **Features:** 40 demographic and employment variables including age, education, occupation, marital status, capital gains/losses, etc.
- **Weight column:** Each record carries a sampling weight representing how many people in the general population that record represents.
...
The dataset is highly imbalanced — roughly 15:1, as shown in Figure 1. This is realistic because high earners are a minority, but requires special handling in model training to avoid the model defaulting to always predicting the majority class.

![Figure 1](figures/fig_label_dist.png)
*Figure 1: Label distribution showing the 15:1 class imbalance between ≤$50K and >$50K earners.*

### 2.3 Key Findings from EDA

- Education is a strong predictor: college graduates and above have substantially higher rates of earning >$50K (Figure 2).
- Age peaks around 40–55 for >$50K earners; very young and very old individuals rarely exceed the threshold (Figure 3).
- Capital gains and dividends are highly skewed — most values are zero, but when non-zero they strongly indicate high income.
- Missing values appear in 9 columns (represented as `?`). Four migration columns have ~99,700 missing values each (~50% of records); the remaining 5 columns including country-of-birth fields, Hispanic origin, state of previous residence have far fewer gaps. All were imputed with mode rather than dropped, for two reasons: dropping would lose up to half the dataset, and the missingness is semantically meaningful — "Not in universe" is the dominant category, so mode imputation correctly assigns these records to the most common valid category rather than inventing a signal.
...
![Figure 2](figures/fig_edu_income.png)
*Figure 2: Percentage of individuals earning >$50K by education level.*

![Figure 3](figures/fig_age_income.png)
*Figure 3: Age distribution by income group.*
...
|---|---|---|
| Missing values | Impute with median / mode | Retains all 199K rows; missingness is not random but correlates with "Not in universe" categories |
| Categorical encoding | Label encoding  | Compatible with tree models (LightGBM); avoids high-dimensional one-hot encoding for 30+ categorical columns |
...
I deliberately used label encoding for tree-based models. LightGBM handles label-encoded categorical variables through split points, which avoids problems arising from any implied ordinal structure. For Logistic Regression, we applied StandardScaler to account for the scale differences introduced by this encoding.
...
The data was divided into three stratified subsets: 72% for training, 8% for validation, and 20% for testing. The validation set was used solely for LightGBM early stopping and for selecting the optimal number of trees, while the test set was reserved entirely for final evaluation. The positive rate of 6.2% was maintained across all three subsets.
...
Both models achieve strong discriminative power (ROC-AUC > 0.93), as shown in Figure 4. LightGBM is preferred as the production model.

With the default decision threshold of 0.5, LightGBM predicts all observations as ≤50K. This pattern is likely driven by the use of `scale_pos_weight = 15`, which improves the model's attention to the minority class during training but also shifts predicted scores downward. As a result, ROC-AUC remains informative because it reflects ranking performance rather than threshold-based classification, whereas the raw scores should not be interpreted as calibrated probabilities.

![Figure 4](figures/fig_classification_eval.png)
*Figure 4: ROC curve and Precision-Recall curve for Logistic Regression, uncalibrated LightGBM, and calibrated LightGBM.*
...
| Calibrated | 0.0 | 1.0 | ~0.062 |

After calibration, the reliability diagram (Figure 5) shows the model is well-calibrated — predicted probabilities track the actual fraction of positives across all probability bins. ROC-AUC is preserved since calibration does not change ranking order.

![Figure 5](figures/fig_calibration.png)
*Figure 5: Left — reliability diagram comparing uncalibrated and calibrated LightGBM. Right — score distribution before and after calibration.*
...
- Unlike the uncalibrated model, which collapses to zero flagged above 0.35, after calibration, thresholds above 0.30 continue to yield predictions, enabling finer precision targeting if needed.

**Recommendation:** Use threshold 0.20–0.25 for direct mail / digital campaigns, and threshold 0.30 for high-touch channels (e.g., personal outreach, premium offers) where cost-per-contact is high. Always apply the calibrated model in production so that scores are interpretable as actual probabilities.

### 4.6 Top Predictive Features

![Figure 6](figures/fig_feature_importance.png)
*Figure 6: Top 20 features by LightGBM split importance.*

Investment income (dividends, capital gains) and human capital (education, occupation) dominate, as shown in Figure 6. A notable insight is that the top two features are both investment income, not salary or occupation. This means the model is primarily identifying *investors* rather than high-salaried employees — a meaningful distinction for the client. People crossing the $50K threshold in this dataset tend to have non-zero capital market exposure, not just high wages. This should inform product strategy: financial and investment products are likely more relevant to this population than purely lifestyle-based premium goods.
...
**Why PCA first?**
- 40 features with label-encoded categoricals create a mixed, high-dimensional space where Euclidean distance is less meaningful.
...
![Figure 7](figures/fig_kmeans_selection.png)
*Figure 7: Elbow method (inertia) and silhouette scores for k = 2 to 9.*

Elbow method and silhouette scores were evaluated for k = 2 to 9 (Figure 7). Silhouette scores for k = 4 are approximately 0.17, rising to ~0.19 at k = 5–6. K = 4 was selected as a deliberate business decision, not because the statistics are indistinguishable. Inspecting k = 5 and k = 6 clusters showed that the additional segments split existing life-stage groups without producing profiles that map to distinct marketing actions. Four segments — children/non-working, prime working adults, near-retirement adults, and moderate mid-career earners — correspond to meaningfully different communication strategies and are directly actionable for the client.

### 5.3 Cluster Profiles

Cluster sizes are reported as population-weighted percentages.

![Figure 8](figures/fig_cluster_heatmap.png)
*Figure 8: Cluster profiles heatmap — z-scored feature means across the four segments.*

![Figure 9](figures/fig_cluster_income.png)
*Figure 9: Percentage of individuals earning >$50K by cluster.*
...
Age profile ~9 years. This group represents minors and non-participants in the labor force. Income is essentially zero. Not a direct marketing target for income-based products, but relevant for family-oriented or education product lines.

**Cluster 1 — Prime Working Adults (44.8%)**
The largest and most economically active segment. Average age 38, higher wage per hour, notable capital gains. Highest share of >$50K earners (12.4%), as shown in Figure 9. This is the primary target for premium retail, investment products, and lifestyle brands.
...
3. Prioritize Cluster 1 individuals with classifier score ≥ 0.25 for premium outreach — at this threshold, ~39% of flagged individuals are genuine >$50K earners and ~77% of all >$50K earners in the population are captured (see Section 4.5).
...
1. U.S. Census Bureau. *Current Population Survey, March 1994 and 1995*. Washington, DC: U.S. Department of Commerce.
